...
console.log('🚀 Initializing Pool B-E');

async function initPoolBE() {
  try {
    ...
    const [poolPDA, poolBump] = PublicKey.findProgramAddressSync(
      [Buffer.from('pool'), tokenB.toBuffer(), tokenE.toBuffer()],
      PROGRAM_ID
    );
    ...
    console.log(`✅ Pool B-E initialized successfully!`);
    ...
    require('fs').writeFileSync('pool-be-info.json', JSON.stringify(poolInfo, null, 2));
    console.log('💾 Pool info saved to pool-be-info.json');

  } catch (error) {
    console.error('❌ Error initializing Pool B-E:', error);
    throw error;
  }
}

initPoolBE().catch(console.error);