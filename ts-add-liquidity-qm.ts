...
/**
 * TypeScript Script: Add Liquidity to Pool Q-M
 * Based on IDL: AddLiquidity (discriminant: 1)
 * Args: amountA (u64), amountB (u64)
 */
async function addLiquidityQM() {
  try {
    console.log("🚀 TypeScript Script: Adding Liquidity to Pool Q-M...");
    
    // Load pool info from previous initialization
    const poolInfo = JSON.parse(fs.readFileSync('pool-qm-info.json', 'utf-8'));
    ...
    // Liquidity addition parameters with 2:1 ratio
    const amountQ = 4_000_000_000; // 4 tokens
    ...
    console.log(`✅ Liquidity added to Pool Q-M successfully!`);
    ...
    fs.writeFileSync("pool-qm-info.json", JSON.stringify(updatedPoolInfo, null, 2));
    console.log("\n💾 Updated pool Q-M info saved to pool-qm-info.json");

  } catch (error) {
    console.error("❌ Error adding liquidity to pool Q-M:", error);
    throw error;
  }
}

// Run the function
addLiquidityQM().catch(console.error);
