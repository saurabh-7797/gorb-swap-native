...
/**
 * TypeScript Script: Add Liquidity to Pool P-Q
 * Based on IDL: AddLiquidity (discriminant: 1)
 * Args: amountA (u64), amountB (u64)
 */
async function addLiquidityPQ() {
  try {
    console.log("🚀 TypeScript Script: Adding Liquidity to Pool P-Q...");
    ...
    const amountP = 5_000_000_000; // 5 tokens
    const amountQ = 5_000_000_000; // 5 tokens
    ...
    console.log(`✅ Liquidity added to Pool P-Q successfully!`);
    ...
    console.log("\n💾 Updated pool P-Q info saved to pool-pq-info.json");

  } catch (error) {
    console.error("❌ Error adding liquidity to pool P-Q:", error);
    throw error;
  }
}
