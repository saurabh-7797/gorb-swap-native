...
/**
 * TypeScript Script: Multihop Swap R → Q → P
 * Based on IDL: MultihopSwap (discriminant: 4)
 * Args: amountIn (u64), minimumAmountOut (u64)
 */
async function multihopSwapRQP() {
  try {
    console.log("🚀 TypeScript Script: Multihop Swap R → Q → P...");
    ...
    // Copy exact pattern from working P→Q→R multihop
    const accounts = [
      ...
      { pubkey: userTokenR, isSigner: false, isWritable: true }, // input = R account (matches token_b)
      { pubkey: intermediateTokenQ, isSigner: false, isWritable: true }, // output = Q account
    ...
    console.log(`✅ Multihop swap R → Q → P completed successfully!`);
    ...
    fs.writeFileSync("multihop-swap-rqp-results.json", JSON.stringify(swapInfo, null, 2));
    console.log("\n💾 Multihop swap results saved to multihop-swap-rqp-results.json");

  } catch (error) {
    console.error("❌ Error in multihop swap R → Q → P:", error);
    throw error;
  }
}

// Run the function
multihopSwapRQP().catch(console.error);
