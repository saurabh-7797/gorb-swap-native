...
/**
 * TypeScript Script: Initialize Pool A-OfficialWrappedSOL
 * Using official wrapped SOL token (So11111111111111111111111111111111111111112)
 */
async function initPoolAOfficialWrappedSOL() {
  try {
    console.log("🚀 TypeScript Script: Initializing Pool A-OfficialWrappedSOL...");
    ...
    const WRAPPED_SOL_MINT = new PublicKey("So11111111111111111111111111111111111111112"); // Official Wrapped SOL
    ...
    console.log(`User Official Wrapped SOL ATA: ${userWrappedSOL.toString()}`);
    ...
    console.log(`Official Wrapped SOL: ${formatTokenAmount(balanceWrappedSOLBefore)} (${balanceWrappedSOLBefore} raw)`);
    ...
    console.log(`Initial Official Wrapped SOL: ${formatTokenAmount(amountWrappedSOL)} Wrapped SOL`);
    ...
    console.log(`✅ Pool A-OfficialWrappedSOL initialized successfully!`);
    ...
    console.log(`Official Wrapped SOL: ${formatTokenAmount(balanceWrappedSOLAfter)} (${balanceWrappedSOLAfter} raw)`);
    ...
    const poolInfo = {
      ...
      tokenWrappedSOL: WRAPPED_SOL_MINT.toString(), // Official Wrapped SOL
      lpMint: lpMintPDA.toString(),
      lpMintBump,
      vaultA: vaultA.toString(),
      vaultWrappedSOL: vaultWrappedSOL.toString(),
      userTokenA: userTokenA.toString(),
      userWrappedSOL: userWrappedSOL.toString(), // Official Wrapped SOL ATA
    ...
    fs.writeFileSync("pool-a-official-wrapped-sol-info.json", JSON.stringify(poolInfo, null, 2));
    console.log("\n💾 Pool A-OfficialWrappedSOL info saved to pool-a-official-wrapped-sol-info.json");

  } catch (error) {
    console.error("❌ Error initializing pool A-OfficialWrappedSOL:", error);
    throw error;
  }
}

// Run the function
initPoolAOfficialWrappedSOL().catch(console.error);
