...
console.log('🚀 Initializing Pool C-E');

async function initPoolCE() {
  try {
    ...
    const [poolPDA, poolBump] = PublicKey.findProgramAddressSync(
      [Buffer.from('pool'), tokenC.toBuffer(), tokenE.toBuffer()],
      PROGRAM_ID
    );
    ...
    console.log(`✅ Pool C-E initialized successfully!`);
    ...
    require('fs').writeFileSync('pool-ce-info.json', JSON.stringify(poolInfo, null, 2));
    console.log('💾 Pool info saved to pool-ce-info.json');

  } catch (error) {
    console.error('❌ Error initializing Pool C-E:', error);
    throw error;
  }
}

initPoolCE().catch(console.error);