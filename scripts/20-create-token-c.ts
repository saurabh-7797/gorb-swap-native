...
async function createTokenC() {
  ...
  // Generate fresh keypair for Token C
  const tokenKeypair = Keypair.generate();
  console.log(`Token C Mint: ${tokenKeypair.publicKey.toString()}`);
  ...
  console.log(`Token C User ATA: ${userToken.toString()}`);
  ...
  console.log(`✅ Token C created successfully! Signature: ${signature}`);
  ...
  console.log(`📊 Token C Balance: ${(Number(balance.amount) / 1e9).toFixed(6)} (${balance.amount} raw)`);
  ...
  fs.writeFileSync("20-token-c-info.json", JSON.stringify(tokenInfo, null, 2));
  console.log("💾 Token C info saved to 20-token-c-info.json");
