...
async function createTokenB() {
  ...
  // Generate fresh keypair for Token B
  const tokenKeypair = Keypair.generate();
  console.log(`Token B Mint: ${tokenKeypair.publicKey.toString()}`);
  ...
  console.log(`Token B User ATA: ${userToken.toString()}`);
  ...
  console.log(`✅ Token B created successfully! Signature: ${signature}`);
  ...
  console.log(`📊 Token B Balance: ${(Number(balance.amount) / 1e9).toFixed(6)} (${balance.amount} raw)`);
  ...
  fs.writeFileSync("20-token-b-info.json", JSON.stringify(tokenInfo, null, 2));
  console.log("💾 Token B info saved to 20-token-b-info.json");
