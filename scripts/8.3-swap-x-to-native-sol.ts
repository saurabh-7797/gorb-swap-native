 ...
 * This swaps Token X for native SOL using the existing native SOL pool
 */
async function swapXToNativeSOL() {
  try {
    ...
    // Swap parameters
    const tokenAmountIn = 0.1 * Math.pow(10, 9); // 0.1 Token X to swap
    
    console.log(`\n🔄 Swap Parameters:`);
    console.log(`Token X Amount In: ${formatTokenAmount(tokenAmountIn)} Token X`);
    console.log(`Direction: Token X → SOL`);
    ...
    const accounts = [
      ...
      { pubkey: USER_TOKEN_X, isSigner: false, isWritable: true }, // user_in_info (Token X)
      { pubkey: userKeypair.publicKey, isSigner: false, isWritable: true }, // user_out_info (SOL - user's main account)
    ...
    data.writeBigUInt64LE(BigInt(tokenAmountIn), 1);
    data.writeUInt8(0, 9); // direction_a_to_b = false (Token X to SOL)
    ...
    console.log(`Exchange Rate: ${solChange / 1e9} SOL per ${formatTokenAmount(tokenXChange)} Token X`);
    ...
    const swapInfo = {
      ...
      exchangeRate: `${solChange / 1e9} SOL per ${formatTokenAmount(tokenXChange)} Token X`
    };

    fs.writeFileSync("swap-x-to-native-sol-info.json", JSON.stringify(swapInfo, null, 2));
    console.log("\n💾 Swap info saved to swap-x-to-native-sol-info.json");
