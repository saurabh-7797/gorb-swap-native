...
/**
 * TypeScript Script: Initialize Pool Q-M
 * Based on IDL: InitPool (discriminant: 0)
 * Args: amountA (u64), amountB (u64)
 */
async function initPoolQM() {
  try {
    console.log("🚀 TypeScript Script: Initializing Pool Q-M...");
    
    // Load Token Q and M info from previous steps
    ...
    const [poolPDA, poolBump] = await PublicKey.findProgramAddress(
      [Buffer.from("pool"), TOKEN_Q_MINT.toBuffer(), TOKEN_M_MINT.toBuffer()],
      AMM_PROGRAM_ID
    );
    ...
    // 5. Pool initialization parameters with 2:1 ratio
    const amountQ = 2_000_000_000; // 2 tokens
    ...
    console.log(`✅ Pool Q-M initialized successfully!`);
    ...
    fs.writeFileSync("pool-qm-info.json", JSON.stringify(poolInfo, null, 2));
    console.log("\n💾 Pool Q-M info saved to pool-qm-info.json");

  } catch (error) {
    console.error("❌ Error initializing pool Q-M:", error);
    throw error;
  }
}

// Run the function
initPoolQM().catch(console.error);
